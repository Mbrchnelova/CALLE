...
print "Interpolating data onto ", mhdfilein_name, " creating ", mhdfileout_name

#HERE PASTE THE CDF-ASCII DUMPED DATA FOR Z1, D1 AND TG1
...
#PART 3: INTERPOLATE ONTO A NEW GRID
...
print "Determining the CFmesh file structure..."
...
#Here we start going through the original CFmesh file and adjusting the variables as we need
for MHDline in MHDlines:
	...
	#Read the lines with the states of the elements
	if l > idx2 and MHDline[0] != "!" and MHDline[0] != "N" and len(MHDline) > 1 and doNOT == False:
		...
		#Take the cell coordinates from the previously stored cell_center array
		...
		#Now we perform interpolation using the RADYN DATA
		for n in range(0, nopoints):
			...
			#This section is kept as the original implementation of IDW, might be useful for some cases
			...
			#This section is for linear interpolation
			if distance < mindist:
				mindist = distance
				n_mindist = n

		#If we use IDW, the new variables are determined as follows
		if (weights != 0.0):
			new_t = new_t/weights
                        new_d = new_d/weights

		#On the other hand, if we interpolate linearly, we override the IDW solution with the values of the nearest data point
	...
	#With determined connectivity, compute the cell-centered coordinates of the elements
	elif l == idx2:
	...
	#Read in the connectivity of the nodes to create the elements
