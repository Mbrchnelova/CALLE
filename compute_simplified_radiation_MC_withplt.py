...
def read_CFmesh_data(MHDinfile):
	...
	for MHDline in MHDlines:
        	...
        	if l > idx2 and MHDline[0] != "!" and MHDline[0] != "N" and len(MHDline) > 1 and doNOT == False:
		        ...
		        rhoi = float(vals[8])/10.
		        rhon = float(vals[9])/10.
		        Vxi = float(vals[10])
		        Vyi = float(vals[11])
		        Vxn = float(vals[12])
		        Vyn = float(vals[13])
			...
			Ey = float(vals[4]) 
			Ez = float(vals[5])
		        Ti = float(vals[14])/100.
		        Tn = float(vals[15])/100.
		        phi = float(vals[6])
		        psi = float(vals[7])
...
def TraceRayPositive(xs, ys, ws, hs, etas, i0, j0, imax, angle, intercepted):
	...
	while stop == False:
		...
		I_mu = I_mu + 100.*math.cos(angle)*s*0.5*(eta0 + eta)
 		eta0 = eta
...
def TraceRayNegative(xs, ys, ws, hs, etas, i0, j0, imax, angle, intercepted):
		...
		intercepted[current_index][0] = intercepted[current_index][0] + 1
                ...
                I_mu = I_mu + 100.0*s*math.cos(angle)*0.5*(eta0 + eta)
                eta0 = eta
...
def computeRadiation(mesh_arr, cells_arr, data_arr, ws, hs, imax, jmax):
	...
	#A. Get chi and eta in every cell
		...
		Q_H, Q_Mg, Q_Ca, esc_HI, esc_MgII, esc_CaII, frac_HI, frac_MgII, frac_CaII = getFits(T_kK, logtau, mc_gcm2)
                ...
                g2 = 2.0*2.0*2.0
                dE = 1.63e-18    #J
                frac_HeI = g2/g1 * math.exp(-(dE)/kB/T_c)
		...
		T_corona = 2.0e6
                chi_c = alpha * rho_gcm**2 * 4.407e23 * A_He * frac_HeI
		Q_coronal = getCoronalLoss(T_corona, ne_cm)
	...
	tau_arr = []
		...
		for v in range(1, no_vertical_cells+1):
			...
			dy = (y_upper - y_lower)*100.0 #transform to cm
		...
		if tau_c == 0.0:
			h_c = hs[c]
			tau_c = 100.0*(-h_c/2.0) * 0.5*(chi_c + chi_c)  
	...
	tau_p_arr = []
		...
		idx_0 = i_0 + (j_0-1)*imax - 1
                ...
                if tau_c == 0.0:
                        h_c = hs[c]
                        tau_c = 100.0*h_c/2.0 * 0.5*(chi_upper + chi_lower)

                tau_p_arr.append(tau_c)



	#B3. Get column mass in every cell
	columnmass_arr = []
		...
		y_find = mesh_arr[idx_find][1]
                ...
                for v in range(2, no_vertical_cells+1):
                        i_v = i_c
                        j_v = v
                        idx_v = i_v + (j_v-1)*imax - 1
                        y_v = mesh_arr[idx_v][1]
	...
	for c in range(0, no_cells):
		...
		Rad_H = -Q_H * esc_HI * frac_HI * A_H * 4.407e23 * rho_gcm * ne_cm
		Rad_Mg = -Q_Mg * esc_MgII * frac_MgII * A_Mg * 4.407e23 * rho_gcm * ne_cm
		Rad_Ca = -Q_Ca * esc_CaII * frac_CaII * A_Ca * 4.407e23 * rho_gcm * ne_cm

		print Q_Mg, esc_MgII, frac_MgII
		...
		Qrad_Mg.append(Rad_Mg)
                ...
                #if printidx%100 == 0:
                #        print"j Qrad_local", j_c, Qrad_local 
		#printidx = printidx +1
		#multiply by 0.1 to get J/s/m3
	...
	for c in range(0, no_cells):
		...
		if printidx%100 == 0:
                	...
                	print j_c, Qrad_coronal_arr[c], Qrad_local_arr[c]
                printidx = printidx + 1
	...
	Qrad_photospheric_arr = []
        ...
        for t in range(1, imax+1):
                ...
                for i in range(0, 9):
                        dangle = pi/2.0/10.0
                        angle = dangle + i*dangle
                        I_ray, intercepted  = TraceRayNegativePhotosphere(mesh_arr[:][0], mesh_arr[:][1], ws, hs, eta_arr, i_t, j_t, imax, jmax, angle, intercepted)
                        I_ray = I_ray / math.cos(angle)
                        I_rays.append([-angle, I_ray])
                ...
                for i in range(0, 9):
                        dangle = pi/2.0/10.0
                        angle = dangle + i*dangle
                        I_ray, intercepted = TraceRayPositivePhotosphere(mesh_arr[:][0], mesh_arr[:][1], ws, hs, eta_arr, i_t, j_t, imax, jmax, angle, intercepted)
                        I_ray = I_ray / math.cos(angle)
                        I_rays.append([angle, I_ray])
        ...
        for c in range(0, no_cells):
			...
			print "no rays in: ", i_c, j_c
                ...
                if printidx%100 == 0:
                        ...
                        print j_c, tau_arr[c], tau_p_arr[c],  Qrad_coronal_arr[c], Qrad_local_arr[c], Qrad_photospheric_arr[c]
                printidx = printidx + 1
...
def writePLT(xs, ys, Qlocal, Qcoronal, Qphotospheric, Ts, rhos, taus, mcs,  Qrad_H, Qrad_Mg, Qrad_Ca, filename, nnodes, coordinates, connectivity):
	...
	for i in range(0, nnodes):
		node_xs.append(coordinates[i][0]/500.)
		node_ys.append(coordinates[i][1]/500.)
...
def getFits(T_kK, logtau, mc):
  ...
  if( logtau > 7.962620685 ) : 
    esc_HI = 0.005263039568 
   


  logtau = math.log10(mc*100)
 
  if( logtau < -9.908828775 ) : 
    esc_CaII = 0.9960242287 
  ...
  if( logtau > 0.9386224154 ) : 
    esc_CaII = 0.008716075608 
    
  if( logtau < -9.892532212 ) : 
    esc_MgII = 0.9975049382 
  ...
  if( T_kK > 29.95008138 ) : 
    frac_HI = 0.005407735845 
    
  if( T_kK < 2.350135199 ) : 
    frac_CaII = 1.045138863 
  ...
  if( T_kK > 29.95624862 ) : 
    frac_CaII = 0.005033261053 
    
  if( T_kK < 2.326568224 ) : 
    frac_MgII = 1.046483256 
...
MHDinfile = open("RADYN_test.CFmesh", "r")
